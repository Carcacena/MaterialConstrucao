...
async function carregarProdutosPDV() {
	   ...
	   try {
        ...
        if (response.ok) {
            ...
            console.error(
                "Erro ao carregar produtos. Status:",
                response.status
            );
        }
    } catch (erro) {
...
function montarArvoreVendasCentro() {
    ...
    raizesOrdenadas.forEach(produtoRaiz => {
        ...
        // 📁 LINHA RAIZ (Corrigido para usar a engrenagem limpa sem %EF%B8%8F) 
        ...
        hierarquia[produtoRaiz].forEach(prod => {
            ...
            const margem = precoCusto > 0
                ? (((precoVenda - precoCusto) / precoCusto) * 100).toFixed(0)
                : "0";

            const estoqueFormatado = prod.aGranel
                ? estoqueAtual.toFixed(3)
                : estoqueAtual.toFixed(0);
            ...
            // Clique no fornecedor abre menu e fecha os outros abertos com segurança total 
            ...
            // Botão Mover
            const btnMoverLinha =
                divMenuBotoes.querySelector("button:nth-child(1)");
            ...
            btnMoverLinha.addEventListener("click", evento => {
                ...
                const quantidadeInput = prod.aGranel
                    ? parseFloat(inputQtd.value)
                    : parseInt(inputQtd.value, 10);

                // 1. Valida a quantidade digitada
                if (
                    !Number.isFinite(quantidadeInput) ||
                    quantidadeInput <= 0
                ) {
                    alert(
                        "Atenção, piá! Informe uma quantidade válida maior que zero."
                    );
                    return;
                }

                /*
                 * 2. Descobre quanto deste mesmo produto
                 * já está reservado no cupom.
                 */
                const itemJaNoCupom =
                ...
                const quantidadeJaNoCupom = Number(
                    itemJaNoCupom?.quantidade || 0
                );

                // 3. Calcula o saldo ainda disponível
                const saldoDisponivel =
                    estoqueAtual - quantidadeJaNoCupom;

                /*
                 * 4. Bloqueia antes de chamar
